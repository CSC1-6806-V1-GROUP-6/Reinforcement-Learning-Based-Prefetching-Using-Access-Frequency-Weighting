...
float FeatureKnowledge::getNormalizedAccessFrequency(State *state) {
    uint32_t offset = state->offset;
    auto it = access_frequency.find(offset);
    if (it == access_frequency.end()) {
        return 0.0f;  
    }
    uint32_t freq = it->second;
    return (float)freq / (freq + 10); 
}

void FeatureKnowledge::updateAccessFrequency(State *state) {
    uint32_t offset = state->offset;  
    access_frequency[offset]++;       
}



void FeatureKnowledge::resetAccessFrequency() {
    float decay_factor = 0.9; 

    for (auto it = access_frequency.begin(); it != access_frequency.end(); ) {
        it->second = static_cast<uint32_t>(it->second * decay_factor); 
...
void FeatureKnowledge::logAccessFrequency() {
    for (auto& entry : access_frequency) {
        uint32_t offset = entry.first;
        uint32_t freq = entry.second;
        MYLOG("Offset %u has been accessed %u times", offset, freq);
    }
}
...
{
    updateAccessFrequency(state1); 
    float AF = getNormalizedAccessFrequency(state1); 
    float beta = 0.5;  


    float R_original = (float)reward;
	float R_new = R_original * (1.0 + beta * AF);
    ...
    for(uint32_t tiling = 0; tiling < m_num_tilings; ++tiling)
    {
        ...
        Qsa1 = Qsa1 + m_alpha * (R_new + m_gamma * Qsa2 - Qsa1);
    	...
    	if (++update_count % 1000 == 0) {
			logAccessFrequency();
        	resetAccessFrequency();
    	}
        MYLOG("<tiling %u> Q(%s,%u) = %0.2f, R = %.2f, Q(%s,%u) = %0.2f, Q(%s,%u) = %0.2f",
              tiling, state1->to_string().c_str(), action1, Qsa1_old, R_new,
    ...
    MYLOG("<feature %s> Q(%s,%u) = %0.2f, R = %.2f, Q(%s,%u) = %0.2f, Q(%s,%u) = %0.2f",
          getFeatureString(m_feature_type).c_str(),
          state1->to_string().c_str(), action1, QSa1_old_overall, R_new,
          state2->to_string().c_str(), action2, QSa2_old_overall,
          state1->to_string().c_str(), action1, QSa1_new_overall);
