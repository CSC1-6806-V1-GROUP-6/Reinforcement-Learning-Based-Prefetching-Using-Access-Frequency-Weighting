...
private:
	...
	FILE *trace;
	std::unordered_map<uint32_t, uint32_t> access_frequency; 
...
public:
	...
	void updateAccessFrequency(State *state); 
    float getNormalizedAccessFrequency(State *state); 
    void resetAccessFrequency(); 
    void logAccessFrequency(); 
};
